...
public abstract class AbstractMinNumberOfCliffs extends AbstractRMSSingleOptionalIntArgumentCommand {
	...
	public SemanticCheckOutput semanticCheck(SemanticCheckInput input) throws AbstractRMSException {
		...
		result.ensureArgumentIsBetween(this.getArgument(0), 0, 0, 9999, true, true);
		
		int min = this.getArgumentAsInt(0, input);
		int max = 8;
		if (this.hasAtLeastOneNextSiblingOfTypes(RMSNodeType.MAX_NUMBER_OF_CLIFFS)) {
			max = ((AbstractExpressionNode)this.getSiblingOfTypes(RMSNodeType.MAX_NUMBER_OF_CLIFFS).getAny()).getAsInt(input);
		}
